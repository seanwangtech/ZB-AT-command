 ...
 * LOCAL FUNCTION
 */
static void AT_ZCL_IR_IdentifyCB( zclIdentify_t *pCmd );
static void AT_ZCL_IR_BasicResetCB( void );
static void AT_ZCL_IR_ProcessIdentifyTimeChange( void );
static void AT_ZCL_IR_OnOffCB( uint8 cmd );
static void AT_ZCL_IR_EP_ENABLE( bool isEnable);

/*********************************************************************
 * ZCL General Profile Callback table
 */
static zclGeneral_AppCallbacks_t AT_ZCL_IR_GEN_CmdCallbacks =
{ 
  AT_ZCL_IR_BasicResetCB,                 // Basic Cluster Reset command
  AT_ZCL_IR_IdentifyCB,                   // Identify command   
  NULL,                                      // Identify Query Response command
  AT_ZCL_IR_OnOffCB,                      // On/Off cluster command
...
void AT_ZCL_IR_Init( byte task_id )
{
  AT_ZCL_IR_TaskID = task_id;
 ...
 * @return      none
 */
uint16 AT_ZCL_IR_event_loop( uint8 task_id, uint16 events )
{
  ...
  {
    while ( (MSGpkt = (afIncomingMSGPacket_t *)osal_msg_receive( AT_ZCL_IR_TaskID )) )
    {
  ...
  if ( events & AT_ZCL_IR_IDENTIFY_TIMEOUT_EVT )
  {
    if (AT_ZCL_IR_IdentifyTime > 0 )
      AT_ZCL_IR_IdentifyTime--;
    AT_ZCL_IR_ProcessIdentifyTimeChange();
...
static void AT_ZCL_IR_BasicResetCB( void )
{
  ...
  AT_ZCL_IR_IdentifyCB( &identifyCmd );
}
...
static void AT_ZCL_IR_IdentifyCB( zclIdentify_t *pCmd )
{
  AT_ZCL_IR_IdentifyTime = pCmd->identifyTime;
  AT_ZCL_IR_ProcessIdentifyTimeChange();
}
...
static void AT_ZCL_IR_ProcessIdentifyTimeChange( void )
{
  if ( AT_ZCL_IR_IdentifyTime > 0 )
  {
    osal_start_timerEx( AT_ZCL_IR_TaskID, AT_ZCL_IR_IDENTIFY_TIMEOUT_EVT, 1000 );
  ...
  else
  {
    ...
    osal_stop_timerEx( AT_ZCL_IR_TaskID, AT_ZCL_IR_IDENTIFY_TIMEOUT_EVT );
  }
}
...
static void AT_ZCL_IR_OnOffCB( uint8 cmd )
{
  ...
  {
    if ( AT_ZCL_IR_OnOff == AT_ZCL_GEN_OFF )
      AT_ZCL_IR_OnOff = AT_ZCL_GEN_ON;
    else
      AT_ZCL_IR_OnOff = AT_ZCL_GEN_OFF;
  }
