...
void AT_handleZCL_EP(void){
  ...
  //ninglvfeihong
  AT_Cmd_EPENABLE(0, ":1,6F\r");//0x6F is 111, force to enable temperature endpoint,because this is temperature endpoint
}
  ...
  case 0: //pressing time less than 5 seconds
    if ( keys & HAL_KEY_SW_1 )
    {
      ...
      if(status==ZSUCCESS){
        ...
        extern void AT_ZCL_TEMP_OnOffCB( uint8 cmd );
        AT_ZCL_TEMP_OnOffCB(2);//toggle switch when the network isn't connected
