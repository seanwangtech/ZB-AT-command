...
void HalKeyConfig(bool interruptEnable, halKeyCBack_t cback)
{
  if ((Hal_KeyIntEnable = interruptEnable))
  {
    ...
    PICTL |= (0x01<<3);              // P2ICONL: Falling edge ints on P2.
    P2IEN |= PUSH1_BV;                // Enable specific P2 bits for ints by bit mask.
 ...
 **************************************************************************************************
 */
HAL_ISR_FUNCTION( usbKeyISR, P2INT_VECTOR )
{
  HAL_ENTER_ISR();
  if(PICTL&(0x01<<3)){//previous seting is flling edge
...
/*
  ...
  */
  ...
  if(PUSH1_SBIT) PICTL |= (0x01<<3); //set to falling edge//anti-shake of the button
  else PICTL &= ~(0x01<<3); //set to rising edge
  HAL_KEY_CLR_INT();
